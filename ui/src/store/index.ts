...
const actions: ActionTree<IState, any> = {
  ...
  [constants.LOGIN]: (
    ...
    { username, password, router }: ILogin,
  ) => {
    ...
    api
      ...
      .then(data =>
        context.commit(constants.LOGIN_SUCCESS, { data, username, router }),
      )
...
const mutations: MutationTree<IState> = {
  [constants.IS_BUSY]: (local: IState, isBusy: boolean) =>
    (local.isBusy = isBusy),
  ...
  [constants.LOGIN_SUCCESS]: (
    local: IState,
    { data, username, router }: ILoginSuccess,
  ) => {
  ...
  [constants.UPDATE_ALL]: (local: IState, payload: IRefreshed) => {
    ...
    local.userOrder = users.map(user => user.idx).sort()
    local.users = users.reduce((users, user) => {
      users[user.idx] = { ...user, perms: user.desc }
      return users
    }, {} as IUsers)

    const apps = payload.apps || []
    local.appOrder = apps.map(app => app.name.toLowerCase())
    local.apps = payload.users.reduce((userApps, user) => {
      userApps[user.idx] = apps.reduce((apps, app) => {
